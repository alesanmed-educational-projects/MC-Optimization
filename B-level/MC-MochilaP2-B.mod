 ...
 *********************************************/
 ...
 maximize sum(c in RgClients) take[c] * Elements[c][0];
 ...
 main{
	...
	for (var c in mochilaModel.RgClients) {
		...
		if(taken[c] == 1) {		
			totalClients += 1;
			totalVolume += Elements[c][1]
			write(c + ", ")
		}
	}	

	writeln("\nCapacidad ocupada: " + totalVolume + "/" + MaxCapacity);
	writeln("Elementos incluídos: " + curr + "/" + totalItems);
	writeln("Clientes incluídos: " + totalClients);
}